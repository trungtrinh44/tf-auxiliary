        ...
        with tf.variable_scope(self.name, reuse=True):
            ...
            for idx, (weight, l, state) in enumerate(zip(self.weights, self.rnn_layers, states)):
                ...
                if self.is_training and drop_s > 0.0:
                    inputs = tf.nn.dropout(x=inputs, keep_prob=1-drop_s, noise_shape=[input_shape[0], input_shape[1], 1], name='drop_s_'+str(idx))
...
class LanguageModel():
    ...
    def build_encoding_model(self):
        ...
        self.layerwise_encode = self.layerwise_last # [tf.concat(out, axis=-1) for out in zip(fw_layerwise_avg, fw_layerwise_max, bw_layerwise_avg, bw_layerwise_max)]
