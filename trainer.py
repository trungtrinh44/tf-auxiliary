...
class Trainer():
    ...
    def build_classifier(self, classifier_configs, folder_name='class_train'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.train_acc = tf.reduce_mean(tf.to_int32(tf.equal(tf.argmax(self.train_classifier.logits, axis=-1, output_type=tf.int32), self.true_y)))
        ...
        self.test_acc = tf.reduce_mean(tf.to_int32(tf.equal(tf.argmax(self.test_classifier.logits, axis=-1, output_type=tf.int32), self.true_y)))
